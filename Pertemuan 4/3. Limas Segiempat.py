...
import math
from Modul import luas_limasSegiempat, volume_limasSegiempat
def hitung():
    try:
        panjang = float(entry_panjang.get())
        lebar = float(entry_lebar.get())
        tinggi = float(entry_tinggi.get())
        luas_permukaan = luas_limasSegiempat (panjang, lebar, tinggi)
        volume = volume_limasSegiempat(panjang, lebar, tinggi)
